...
describe("Complete Workflow E2E Tests", () => {
  ...
  test("Complete workflow: setup → sync → convert → mcp integration", async () => {
...
You are a test agent used for end-to-end workflow validation.`;
    ...
    const mcpConfigResult = execSync(`bun run ${CLI_PATH} mcp configure claude-desktop --project ${testProjectDir}`, {
      encoding: 'utf8',
      timeout: 15000
    });
  ...
  test("File watching performance test", async () => {
...
Test content for file watching.`);
    ...
    const watcherProcess = spawn('bun', ['run', CLI_PATH, 'watch', 'start', '--project', testProjectDir], {
      stdio: 'pipe',
      detached: true
    });
...
Modified test content for file watching.`);
  ...
  test("Agent parsing performance benchmark", async () => {
...
This is agent number ${i} in the performance test suite.`;
    ...
    console.log(`Parsed ${agentCount} agents in ${parseTime}ms (${timePerAgent.toFixed(2)}ms per agent)`);
  ...
  test("Security validation - input sanitization", async () => {
  ...
  test("Error handling and recovery", async () => {
    ...
    expect(existsSync(join(invalidProject, '.codeflow'))).toBe(true);
