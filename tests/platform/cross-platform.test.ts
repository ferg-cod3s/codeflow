...
describe("Cross-Platform Compatibility", () => {
  ...
  test("resolves Claude Desktop config path correctly", () => {
    ...
    if (process.platform === "darwin") {
      expectedConfigPath = path.join(home, "Library/Application Support/Claude/claude_desktop_config.json");
      expect(expectedConfigPath).toContain("Library");
      expect(expectedConfigPath).toContain("Application Support");
    } else if (process.platform === "win32") {
      expectedConfigPath = path.join(process.env.APPDATA || "", "Claude/claude_desktop_config.json");
...
describe("Platform-Specific Path Resolution", () => {
  ...
  test("constructs MCP config paths correctly", () => {
    ...
    if (process.platform === "darwin") {
      mcpConfigPath = path.join(os.homedir(), "Library", "Application Support", "Claude", "claude_desktop_config.json");
    } else if (process.platform === "win32") {
      mcpConfigPath = path.join(os.homedir(), "AppData", "Roaming", "Claude", "claude_desktop_config.json");
    } else {
