...
inputs:
  - name: target
    description: Target system or application for metrics collection
    type: string
    required: true
  - name: metrics_type
    description: Type of metrics to collect (application, infrastructure, business, user_experience)
    type: string
    default: application
  ...
  - name: alert_thresholds
    ...
    default: {}
  - name: dashboard_type
    description: Type of dashboards to create (operational, executive, technical)
    type: array
    default: ['operational']
outputs:
...
cache_strategy:
  type: tiered
  ttl: 3600
  key_pattern: 'metrics:{target}:{metrics_type}'
success_signals:
...
failure_modes:
  - Metrics collection service unavailable
  - Insufficient permissions for monitoring
  - Target system inaccessible
