...
- **sync-global command**: Fixed commands directory sync failure with enhanced error handling and logging
- **agent allowed directories**: Resolved MCP security configuration issues for all agents
- **YAML parser**: Enhanced to properly handle array fields like `allowed_directories`
- **agent registry**: Added missing `.opencode/agent` directory to priority loading list
...
- **MCP Security**: Implemented comprehensive allowed directories security controls for all agents
- **Enhanced Agent Spawner**: Added path validation and security restrictions to agent execution
- **Improved Error Handling**: Better diagnostic logging for sync operations and agent loading
