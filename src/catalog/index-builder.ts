...
export class CatalogIndexBuilder {
  ...
  private extractTags(frontmatter: string, category: string): string[] {
    ...
    const categoryMatch = frontmatter.match(/^category:\s*(.+)/mi);
    if (categoryMatch) {
  ...
  async save(index: CatalogIndex): Promise<void> {
    ...
    console.log(`📊 Stats: ${index.stats.total_items} total items (${index.stats.agents} agents, ${index.stats.commands} commands)`);
  }
