...
export async function convertAll(projectPath?: string, options: { validate?: boolean; dryRun?: boolean } = {}) {
  ...
  const conversions = [
    ...
    { source: join(codeflowDir, "claude-agents"), target: join(codeflowDir, "agent"), format: 'base' as const },
    ...
    { source: join(codeflowDir, "opencode-agents"), target: join(codeflowDir, "agent"), format: 'base' as const },
