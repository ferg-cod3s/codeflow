...
function getWarpConfigPath(): string {
  ...
  switch (platform) {
    case 'darwin': // macOS
      return join(home, 'Library/Application Support/Warp/warp_config.json');
    case 'win32': // Windows
      return join(process.env.APPDATA || '', 'Warp/warp_config.json');
    default: // Linux and others
      return join(home, '.config/Warp/warp_config.json');
  }
}
...
function getCursorConfigPath(): string {
  ...
  switch (platform) {
    case 'darwin': // macOS
      return join(home, 'Library/Application Support/Cursor/User/globalStorage/workspace.json');
    case 'win32': // Windows
      return join(process.env.APPDATA || '', 'Cursor/User/globalStorage/workspace.json');
    default: // Linux and others
      return join(home, '.config/Cursor/User/globalStorage/workspace.json');
  }
}
...
async function loadWarpConfig(): Promise<any> {
  ...
  try {
    ...
    CLIErrorHandler.displayWarning('Could not parse Warp config', [
...
async function loadCursorConfig(): Promise<any> {
  ...
  try {
    ...
    CLIErrorHandler.displayWarning('Could not parse Cursor config', [
      'Check if the config file is valid JSON',
      'Verify file permissions',
      'The config will be recreated if needed',
    ]);
...
export async function mcpServer(
  ...
  try {
    switch (action) {
      ...
      case 'stop': {
        ...
        try {
          ...
          CLIErrorHandler.displayWarning('No running MCP Server found', [
      ...
      case 'status': {
        ...
        try {
          if (process.platform === 'win32') {
            const output = statusExecSync('tasklist /fi "IMAGENAME eq bun.exe" /fo csv', {
              encoding: 'utf-8',
            });
    ...
    CLIErrorHandler.handleCommonError(new Error('MCP server operation failed'), 'mcp-server');
  }
}
...
export async function mcpConfigure(client: string, options: { remove?: boolean } = {}) {
  ...
  try {
      ...
      case 'claude': {
        ...
        if (options.remove) {
          if (config.mcpServers && config.mcpServers['codeflow-tools']) {
            ...
            CLIErrorHandler.displaySuccess('Removed codeflow from Claude Desktop configuration', [
              'Restart Claude Desktop for changes to take effect',
              "Use 'codeflow mcp configure claude-desktop' to reconfigure",
            ]);
          } else {
            CLIErrorHandler.displayWarning('Codeflow is not configured in Claude Desktop', [
              'Nothing to remove',
        ...
        await saveClaudeConfig(config);
        CLIErrorHandler.displaySuccess('Configured Claude Desktop for codeflow MCP integration', [
          'Restart Claude Desktop for changes to take effect',
      ...
      case 'warp': {
        ...
        if (options.remove) {
          if (warpConfig.mcpServers && warpConfig.mcpServers['codeflow-tools']) {
            ...
            CLIErrorHandler.displaySuccess('Removed codeflow from Warp configuration', [
              'Restart Warp or reload settings for changes to take effect',
              "Use 'codeflow mcp configure warp' to reconfigure",
            ]);
          } else {
            CLIErrorHandler.displayWarning('Codeflow is not configured in Warp', [
              'Nothing to remove',
        ...
        await saveWarpConfig(warpConfig);
        CLIErrorHandler.displaySuccess('Configured Warp for codeflow MCP integration', [
      ...
      case 'cursor': {
        ...
        if (options.remove) {
          if (cursorConfig.mcpServers && cursorConfig.mcpServers['codeflow-tools']) {
            ...
            CLIErrorHandler.displaySuccess('Removed codeflow from Cursor configuration', [
              'Restart Cursor for changes to take effect',
              "Use 'codeflow mcp configure cursor' to reconfigure",
            ]);
          } else {
            CLIErrorHandler.displayWarning('Codeflow is not configured in Cursor', [
              'Nothing to remove',
        ...
        await saveCursorConfig(cursorConfig);
        CLIErrorHandler.displaySuccess('Configured Cursor for codeflow MCP integration', [
          'Restart Cursor',
          'Navigate to your project directory',
    ...
    CLIErrorHandler.handleCommonError(new Error('MCP configure operation failed'), 'mcp-configure');
  }
}
...
export async function mcpList(): Promise<void> {
  ...
  console.log(
    "  • Claude.ai: Use 'codeflow setup . --type claude-code' (creates .claude/commands/)"
  );
  console.log('');
