 ...
 */
export function getPermissionTemplate(
  role: string,
  format: 'opencode' | 'claude'
): AgentRoleTemplate[typeof format] {
  ...
  if (format === 'opencode') {
    ...
    for (const tool of perms) {
      if (!expectedTools.includes(tool)) {
        violations.push(
          `Agent has '${tool}' permission but role ${role} should not have this tool`
        );
      }
    }
  }
 ...
 */
export function getRoleStatistics(): Record<
  string,
  { toolCount: number; hasWebAccess: boolean; hasWriteAccess: boolean }
> {
  const stats: Record<
    string,
    { toolCount: number; hasWebAccess: boolean; hasWriteAccess: boolean }
