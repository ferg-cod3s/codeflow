...
class SyncManager {
  ...
  async sync(options: SyncOptions = {}): Promise<void> {
    ...
    const agentResult = await this.syncDirectory(
      paths.agents.source,
      agentTargets,
      options
    );
    ...
    const commandResult = await this.syncDirectory(
      paths.commands.source,
      commandTargets,
      options
    );
  ...
  async validateAll(): Promise<void> {
    ...
    const agentFiles = existsSync(paths.agents.source)
      ? await readdir(paths.agents.source)
      : [];
