...
- Document generated artifacts policy for `.opencode/` and `.claude/` (do not commit; see .gitignore)
- Add OpenCode model allowlist and guidance to omit `model:` to use platform defaults
- Mention drift detection workflow for uncommitted changes and invalid model usage
...
## Generated Artifacts: `.opencode/` and `.claude/` (Do Not Commit)

- These directories are generated by Codeflow/Agentic commands (e.g., setup, sync, pull, convert) and are treated as build artifacts.
- They will be overwritten/regenerated as needed. Do not edit them directly for long-lived changes; edit the source-of-truth files under `command/` and `codeflow-agents/`.
- They must not be committed to git. This policy is enforced via `.gitignore`.
- If these folders were accidentally added to git, remove them from the index: `git rm -r --cached .opencode .claude` and re-run the appropriate sync/setup command.

## OpenCode Model Allowlist

- Only the following models are valid for OpenCode agents:
  - `opencode/code-supernova`
  - `opencode/grok-code`
- Using any other model identifier will be flagged by validation/drift checks.
- Omitting the `model:` field is allowed and preferred to use platform defaults; the allowlist is still enforced at validation time.

## Drift Detection Workflow (Overview)

The workflow now includes drift detection that:
- Flags uncommitted changes within generated artifact directories (`.opencode/`, `.claude/`).
- Validates agent `model:` usage against the allowlist and reports violations.
- Provides remediation guidance (e.g., regenerate artifacts with `codeflow sync`, update source-of-truth files, or remove accidental edits).
...
- `depth`: `"medium"`
- `model`: omitted (platform default; allowlist enforced)
- `temperature`: `0.1`
