...
- organization
- mapping
primary_objective: Universal File & Directory Location Specialist - produces a structured,
  comprehensive, classification-oriented map of all files and directories relevant
  to a requested feature/topic WITHOUT reading file contents.
anti_objectives:
...
tools:
  ...
  glob: true
  list: true
  read: true   # Required for grep to work
  edit: false
  write: false
  bash: false
  webfetch: false
...
permission:
  ...
  edit: deny
  write: deny
  read: allow   # Allow reading for grep searches
  webfetch: deny
  patch: deny
  grep: allow
  glob: allow
