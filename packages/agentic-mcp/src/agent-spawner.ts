...
export function buildAgentPrompt(agent: Agent, task: string): string {
  ...
  if (agent.mode === 'subagent') {
    ...
    sections.push('You are operating as a specialized subagent. Focus on your specific expertise and provide targeted, actionable output.');
...
export function validateAgentExecution(agentId: string, registry: Map<string, Agent>): AgentValidationResult {
  const agent = registry.get(agentId);
