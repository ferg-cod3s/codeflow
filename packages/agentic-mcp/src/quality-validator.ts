...
export function calculateQualityMetrics(
  ...
  const overallScore = Math.round(
    completeness * 0.3 + accuracy * 0.3 + coverage * 0.2 + consistency * 0.2
  );
...
function calculateCompleteness(workflowResult: MultiPhaseWorkflowResult): number {
  ...
  const agentCompletion =
    (workflowResult.summary.successfulAgents / workflowResult.summary.totalAgents) * 100;
...
function calculateCoverage(
  ...
  const uniqueAgents = new Set(
    workflowResult.phases.flatMap((p) => p.results.map((r) => r.agentId))
  ).size;
  const varietyScore = Math.min(uniqueAgents * 10, 30); // Max 30 points for 3+ agents

  const sequentialPhases = workflowResult.phases.filter(
    (p) => p.type === 'sequential' && p.status === 'completed'
  ).length;
...
function evaluateQualityGates(
  ...
  gates.push({
    ...
    threshold: 60,
    message:
      metrics.coverage >= 60 ? 'Analysis coverage is sufficient' : 'Limited analysis coverage',
...
function generateQualityRecommendations(
  ...
  if (metrics.completeness < 70) {
    recommendations.push(
      'Increase workflow completeness by ensuring all phases execute successfully'
    );
  }

  if (metrics.accuracy < 75) {
    recommendations.push(
      'Improve agent accuracy by validating agent configurations and dependencies'
    );
  }

  if (metrics.coverage < 60) {
    recommendations.push(
      'Enhance coverage by including more specialized agents or additional research phases'
    );
  }

  if (metrics.consistency < 70) {
    recommendations.push(
      'Improve consistency by ensuring proper dependency resolution and uniform agent quality'
    );
  }
  ...
  if (metrics.overallScore < 80) {
    recommendations.push(
      'Consider re-running workflow with adjusted parameters or additional agents'
    );
  }
...
export function formatQualityMetrics(metrics: QualityMetrics): string {
  ...
  for (const gate of metrics.gates) {
    ...
    lines.push(
      `  ${status} ${icon} ${gate.name}: ${gate.score}/${gate.threshold} - ${gate.message}`
    );
  }
