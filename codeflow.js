...
class ValidationEngine {
  ...
  validateOpenCode(agent) {
    ...
    this.validateTemperature(agent.temperature, 0, 2, errors2);
    this.validateToolsObject(agent.tools, errors2, warnings);
    this.validatePermissions(agent.permission, errors2);
  ...
  validatePermissions(permission, errors2) {
    if (!permission)
      return;
    ...
    for (const [action, permValue] of Object.entries(permission)) {
      if (!["allow", "ask", "deny"].includes(permValue)) {
        errors2.push({
    ...
    this.validateRequiredPermissions(permission, errors2);
    this.validatePermissionConsistency(permission, errors2);
  }
  validateRequiredPermissions(permission, errors2) {
  ...
  validatePermissionConsistency(permission, errors2) {}
...
function normalizePermissionFormat(frontmatter) {
  if (frontmatter.tools && typeof frontmatter.tools === "object") {
    ...
    const { edit, bash, patch, read, grep, glob, list, webfetch, write, ...cleanFrontmatter } = frontmatter;
    return {
      ...cleanFrontmatter,
...
async function parseAgentsFromDirectory(directory, format) {
  ...
  try {
    ...
    for (const item of files) {
      const itemPath = join2(directory, item);
...
class FormatConverter {
  baseToClaudeCode(agent) {
    ...
    const claudeCodeFrontmatter = {
      ...
      tools: baseAgent.tools ? Object.entries(baseAgent.tools).filter(([, enabled]) => enabled).map(([tool]) => tool).join(", ") : undefined
    };
    return {
      ...agent,
      format: "claude-code",
  ...
  baseToOpenCode(agent) {
    ...
    const openCodeFrontmatter = {
      ...
      permission: baseAgent.permissions?.opencode ? this.convertBaseOpenCodePermissions(baseAgent.permissions.opencode) : baseAgent.tools ? this.convertToolsToPermissions(baseAgent.tools) : undefined,
  ...
  convertAll(sourceDir, targetFormat, outputDir) {}
  ...
  if (existsSync(sourceCommandDir)) {
    ...
    for (const file of mdFiles) {
      const possibleLocations = [
        join(targetBase, "command"),
        join(resolvedPath, ".claude", "commands"),
        join(resolvedPath, ".cursor", "mcp.json")
      ];
...
// src/cli/setup.ts
init_agent_parser();
import { readdir as readdir2, mkdir as mkdir2, copyFile, writeFile, readFile as readFile2 } from "fs/promises";
...
function getCommandSourceDirs(sourcePath, targetDir) {
  const sourceDirs = [];
  sourceDirs.push(join2(sourcePath, "command"));
  if (targetDir.includes(".claude")) {
    sourceDirs.push(join2(sourcePath, "claude-agents"));
  } else if (targetDir.includes(".opencode")) {
    sourceDirs.push(join2(sourcePath, "opencode-agents"));
  }
  return sourceDirs;
}
function getAgentSourceDirs(sourcePath, targetFormat) {
  const sourceDirs = [];
  sourceDirs.push(join2(sourcePath, "codeflow-agents"));
...
async function copyCommands(sourcePath, targetPath, setupDirs) {
  let fileCount = 0;
  for (const setupDir of setupDirs) {
    const targetDir = join2(targetPath, setupDir);
          ...
          try {
            ...
            for (const file of files) {
              if (file.isFile() && file.name.endsWith(".md")) {
                try {
                  const sourceFile = join2(sourceDir, file.name);
                  const targetFile = join2(targetDir, file.name);
...
async function copyAgentsWithConversion(sourcePath, targetDir, targetFormat) {
  ...
  try {
    ...
    for (const agent of convertedAgents) {
      try {
        ...
        const targetFile = join2(targetDir, filename);
...
async function createProjectReadme(projectPath, projectType) {
  const readmePath = join2(projectPath, "README.md");
  let readmeContent = "";
  if (existsSync2(readmePath)) {
    const existingContent = await readFile2(readmePath, "utf-8");
...
`;
    ...
    readmeContent = `# ${basename2(projectPath)}
...
async function setup(projectPath, options = {}) {
  ...
  if (options.type) {
    ...
    const hasClaude = existsSync2(join2(inputPath, ".claude"));
    const hasOpenCode = existsSync2(join2(inputPath, ".opencode"));
  ...
  if (!options.force) {
    const hasExistingSetup = setupDirs.some((dir) => existsSync2(join2(inputPath, dir)));
    if (hasExistingSetup) {
  ...
  const codeflowDir = join2(import.meta.dir, "../..");
...
`);
  ...
  for (const agent of convertedAgents) {
    try {
      const filename = `${agent.frontmatter.name}.md`;
      const filePath = join3(target, filename);
      const serialized = serializeAgent(agent);
...
import { readdir as readdir4, copyFile as copyFile3, mkdir as mkdir5 } from "fs/promises";
...
import path, { basename as basename3 } from "path";
import os from "os";
class CanonicalSyncer {
  ...
  async loadManifest() {
    ...
    return JSON.parse(await readFile3(manifestPath, "utf-8"));
  }
  ...
  async validateAllAgents(agents, options, result) {
    ...
    for (const agent of agents) {
      ...
      try {
        if (!sourceFile || !existsSync4(sourceFile)) {
          result.errors.push({
            agent: agent.name,
        ...
        const { ValidationEngine: ValidationEngine2 } = await Promise.resolve().then(() => exports_validation_engine);
  ...
  async syncCommandsToGlobal(_options) {
    const tempFiles = [];
    const codeflowRoot = path.join(process.cwd());
    const sourceCommandDir = path.join(codeflowRoot, "command");
    if (!existsSync4(sourceCommandDir)) {
      return tempFiles;
    }
    try {
      const files = await readdir3(sourceCommandDir);
      ...
      for (const file of mdFiles) {
        ...
        for (const targetPath of globalTargetPaths) {
          ...
          try {
            const targetDir = path.dirname(targetPath);
            await mkdir4(targetDir, { recursive: true });
            await copyFile2(sourceFile, tempPath);
            tempFiles.push(tempPath);
          } catch (error) {
...
async function sync(projectPath, options = {}) {
  ...
  const manifestPath = join4(process.cwd(), "AGENT_MANIFEST.json");
  const hasManifest = existsSync5(manifestPath);
  if (hasManifest) {
...
\uD83D\uDCCA Summary: ${result.synced.length}/${totalProcessed} files synced successfully`);
  ...
  const codeflowDir = join4(import.meta.dir, "../..");
  console.log(`\uD83D\uDD04 Syncing from: ${codeflowDir}`);
  console.log(`\uD83D\uDCE6 Syncing to: ${resolvedPath}
`);
  let totalSynced = 0;
  try {
    const sourceCommandDir = join4(codeflowDir, "command");
    const targetCommandDir = join4(resolvedPath, ".opencode/command");
    if (existsSync5(sourceCommandDir)) {
      if (!existsSync5(targetCommandDir)) {
        await mkdir5(targetCommandDir, { recursive: true });
        console.log(`  \u2713 Created directory: .opencode/command`);
      }
      const files = await readdir4(sourceCommandDir);
      const mdFiles = files.filter((f) => f.endsWith(".md"));
      for (const file of mdFiles) {
        const sourceFile = join4(sourceCommandDir, file);
        const targetFile = join4(targetCommandDir, file);
        try {
    ...
    const sourceAgentDir = join4(codeflowDir, "codeflow-agents");
    const targetAgentDir = join4(resolvedPath, ".opencode/agent");
    if (existsSync5(sourceAgentDir)) {
      if (!existsSync5(targetAgentDir)) {
        await mkdir5(targetAgentDir, { recursive: true });
      ...
      for (const agent of convertedAgents) {
        try {
          const filename = `${agent.frontmatter.name}.md`;
          const targetFile = join4(targetAgentDir, filename);
          const serialized = serializeAgent(agent);
...
var package_default = {
  ...
  author: "ferg.cod3s <john.ferguson@jferguson.info>",
  license: "MIT",
  files: [
    "src",
    "mcp",
  ...
  scripts: {
    ...
    prepublishOnly: "bun run typecheck && bun run test && bun run lint || echo 'Lint warnings found but continuing with publish'",
    prepack: "bun run typecheck",
    "build:cli": "bun build src/cli/index.ts --target bun",
    prepare: "husky"
  },
  dependencies: {
...
function getFormatDirectory(format, projectPath) {
  const codeflowRoot = join5(import.meta.dir, "../..");
  switch (format) {
    case "base":
      return join5(codeflowRoot, "codeflow-agents");
    case "claude-code":
      const projectClaudeDir = join5(projectPath, ".claude", "agents");
      return existsSync7(projectClaudeDir) ? projectClaudeDir : join5(codeflowRoot, "claude-agents");
    case "opencode":
      const projectOpenCodeDir = join5(projectPath, ".opencode", "agent");
      return existsSync7(projectOpenCodeDir) ? projectOpenCodeDir : join5(codeflowRoot, "opencode-agents");
    default:
...
var command = args[0];
if (values.version) {
  console.log(`Codeflow ${package_default.version}`);
  process.exit(0);
}
