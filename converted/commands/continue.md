...
template: >-
  ...
  **Input**: $ARGUMENTS
  ...
  Resume the previous task from where it left off. This prompt analyzes
  conversation history to identify the last completed step and continues with
  the next pending action without repeating prior work.


  ## Purpose


  Provide seamless task continuation across sessions by maintaining context and
  state from previous executions. Enables efficient workflow resumption without
  manual restatement of requirements or progress.
  ...
  Codeflow provides the continue command as a structured workflow command that
  can be used with either OpenCode or Claude Code:
  ...
  1. **Locate Last Session**: Identify most recent session or specified session
  ID
