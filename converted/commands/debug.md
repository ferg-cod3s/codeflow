---
name: debug
description: Debug issues with issue reproduction, root cause analysis, fix
...
template: >-
  ...
  **Input**: $ARGUMENTS
  ...
  Execute a comprehensive debugging workflow with systematic issue reproduction,
  root cause analysis, fix implementation, and regression test creation to
  ...
  Systematically debug and resolve issues through structured reproduction,
  analysis, implementation, and validation phases to ensure reliable and lasting
  fixes.
  ...
  1. **Check Cache First**: Query cache for similar issue patterns using issue
  context hash

  2. **Read Issue Details**: Parse issue description and gather all available
  context

  3. **Collect Evidence**: Gather logs, stack traces, metrics, and error
  messages
  ...
  7. **Security Review**: Validate fix doesn't introduce security
  vulnerabilities
  ...
  1. **Comprehensive Investigation**: Use multiple domain experts for thorough
  analysis
  ...
  - **Content-based**: Invalidate when issue characteristics change
  significantly
