...
async function installWarpMCP() {
  ...
  switch (plat) {
    case 'darwin': // macOS
      ...
      break;
    case 'win32': // Windows
      configPath = join(process.env.APPDATA || join(home, 'AppData', 'Roaming'), 'Warp', 'mcp_config.json');
      break;
    default: // Linux
...
async function main() {
  ...
  console.log(`${colors.bright}${colors.cyan}     Codeflow MCP Installation & Setup${colors.reset}`);
  console.log(`${colors.bright}${colors.cyan}═══════════════════════════════════════════════════${colors.reset}`);
  console.log('');
