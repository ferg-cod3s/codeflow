...
// Fallback manual frontmatter parser
function extractBasicFrontmatter(yamlContent: string): any {
...
const SPECIALIZATION_MAPPING: Record<
  string,
  {
    role: string;
    purpose: string;
...
function extractAgentInfo(filePath: string): AgentInfo {
  ...
  if (frontmatter.tools) {
    if (Array.isArray(frontmatter.tools)) {
      ...
      tools = Object.keys(frontmatter.tools).filter((tool) => frontmatter.tools[tool]);
    } else {
      tools = DEFAULT_TOOLS;
    }
  } else {
...
function extractCapabilities(content: string): string[] {
  ...
  for (const pattern of capabilityPatterns) {
    ...
    if (match) {
      ...
      capabilities.push(...bullets.map((b) => b.replace(/^- /, '').trim()));
    }
  }
...
function extractWhenToUse(content: string): string[] {
  ...
  if (whenToUseMatch) {
    ...
    useCases.push(...bullets.map((b) => b.replace(/^- /, '').trim()));
  }
...
function extractApproach(content: string): string[] {
  ...
  for (const pattern of approachPatterns) {
    ...
    if (match) {
      ...
      approach.push(...steps.map((s) => s.replace(/^\d+\.\s+|^- /, '').trim()));
    }
  }
...
${whenToUse.map((use) => `- ${use}`).join('\n')}

## Approach
${approach.map((step, index) => `${index + 1}. **${step.split(' - ')[0]}** - ${step.split(' - ')[1] || step}`).join('\n')}
